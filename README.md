...
[![Live Demo](https://img.shields.io/badge/Live-Demo-brightgreen)](https://adamkingry.github.io/pendulum-cipher/)
...
Before generating the keystream, both pendulums are simulated for a configurable "warmup" period (default 3 seconds). This allows the chaos to develop:

```
t=0.00s: Pendulums start from key-derived positions
t=0.02s: First simulation step
t=0.04s: Second simulation step
  ...
t=3.00s: Warmup complete, chaos fully developed
```

**Important:** The warmup time affects the final keystream. Different warmup times produce different ciphertexts, even with the same key!

### Step 3: Keystream Generation

After warmup, the keystream is generated by repeatedly:
...
2. Use the **same warmup time** (same chaos development)
...
1. Open `pendulum_cipher.html` in a browser
2. Enter your encryption key
3. Adjust the "Chaos Warmup Time" slider (affects output!)
4. Enter plaintext and click **ENCRYPT**
5. Copy the hex ciphertext
6. To decrypt: paste hex, use same key + warmup time, click **DECRYPT**
